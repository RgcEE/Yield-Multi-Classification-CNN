...
Description — Ablation Run A: CoordConv only, no SE attention.

Extends yield_resnet_focal.py with a single addition:

    CoordConv channels — two spatial coordinate maps (row and column,
    normalized to [0, 1]) are concatenated to the wafer image before
    entering the network. The first ResidualBlock takes in_ch=3 instead
    of 1. SEBlock is absent; this run isolates whether coordinate channels
    alone are stable and beneficial.

Everything else — data loading, augmentation, sampler, optimizer,
ResidualBlock structure, and loss — is identical to yield_resnet_focal.py.

Usage:
    python yield_coord_only.py
...
CHECKPOINT_PATH = Path(__file__).parent.parent / "checkpoints" / "best_coord_only.pt"
...
class WaferDataset(Dataset):
    ...
    def _add_coord_channels(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Both coord tensors are created on the same device as x so the cat is
        always device-consistent. The full (3, H, W) tensor is moved to GPU
        by the training loop after collation, never split across devices.
...
class ResidualBlock(nn.Module):
    """
    Two conv layers with a skip connection. No SE attention (ablation Run A).
...
class WaferResNet(nn.Module):
    """
    WaferResNet with CoordConv input channels, no SE attention.

    Channel progression: 32→64→128→256. Each stage uses stride=2 for
    downsampling. First block accepts in_ch=3 (wafer + row coords + col coords).
...
def main():
    ...
    log_run(
        ...
        notes        = "ResidualBlocks + SiLU + FocalLoss + CoordConv (no SE)",
    )
